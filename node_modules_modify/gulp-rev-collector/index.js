    ...
    if (ext === '.json') {
        ...
        if (_.isObject(json)) {
            ...
            Object.keys(json).forEach(function (key) {
				...
				if (typeof(json[key].path)=="undefined" || !_.isString(json[key].path) || path.basename(json[key].path) !== path.basename(key) ) {
                    isRev = 0;
                }
            });
    ...
    return through.obj(function (file, enc, cb) {
        ...
        for (var key in manifest) {		
            ...
            if (opts.replaceReved) {
				var pattern=escPathPattern( (path.dirname(key) === '.' ? '' : closeDirBySep(path.dirname(key)) ) + path.basename(key, path.extname(key)) )
                            + opts.revSuffix
			...
			if(opts.paramPattern){
				for(var i=0;i<patterns.length;i++){
					patterns[i]+=opts.paramPattern;
				}
			}
            if ( dirReplacements.length ) {
                    ...
                    patterns.forEach(function (pattern) {
                        changes.push({
                            ...
                            replacement: _.isFunction(dirRule.dirRpl)
                                            ? dirRule.dirRpl(manifest[key].path)
                                            : closeDirBySep(dirRule.dirRpl) + manifest[key].path
                        });
                    });
                });
            } else {
                patterns.forEach(function (pattern) {
                    ...
                    changes.push({
                        ...
                        replacement: '$1' + manifest[key].path,
        ...
        mutables.forEach(function (file){
            ...
            if (!file.isNull()) {
                ...
                changes.forEach(function (r) {				
					...
					var ori=src;
					src=src.replace(r.regexp,function(url){
					var version="v="+r.version;					
					if(url.indexOf("?")==-1)
						return url+"?"+version;// url.replace(r.regexp, r.replacement)+"?"+version;
					else{
						var reg=/(\?|&)v=[^&"'\) ]*/i;						
						if(reg.test(url))
							url=url.replace(reg,"$1"+version);
						else{								
								url+="&"+version;								
							}
						return url;
					...
					if(ori!=src){
						var fPath=file.base+r.path;
						if(sFiles.indexOf(fPath)==-1)
							sFiles.push(fPath);
					}
                });
				if(src!=file.contents){
...
revCollector.getResource=function(){return sFiles;}
